...
- **`--open <path>` on the CLI.** `npx cc-tap --open /sessions/<id>` opens the browser on that page rather than the overview, so a tool that hands cc-tap one session (a pulled Coder task, a link from a team dashboard) lands on it. The path reaches a shell (`open` / `start` / `xdg-open`), so only a plain app path is taken (`bin/open-path.js`); anything else is ignored with a warning.
...
- **Search in the conversation.** A loupe at the bottom of the Replay tab, and at the top of a sub-agent or orchestrator drawer, opens a find bar over the whole conversation — every turn, not only the ones on screen — searching the turn text, the thinking, the tool names and inputs, and the tool results. It counts the matching turns and steps through them with the arrows, Enter and Shift+Enter. `⌘F` opens it on the text selected in the conversation; `Esc` clears the query, then folds the bar. Folded, it searches nothing and keeps its query for the next time.
  - The query is exact text by default: as typed, in one piece. The `""` toggle turns that off, and every word is then looked for on its own, in any order, a word with no exact match matched loosely — so a typo such as `rbase` still finds `rebase` — and a quoted part must appear as that phrase. An `Aa` toggle matches the case. Both apply to the hits and to the highlight alike.
  - Every occurrence is painted in the text through the CSS highlight registry, the one under the arrows more strongly, across a line break or an inline `code` element. A thinking block, a long response, a tool call or a tool result holding the hit opens itself, and the jump lands on the matched text rather than on the top of the turn.
  - The turn list is virtualised, in the Replay and in the drawers, so only what is on screen is mounted: a jump costs the same wherever it lands. On a 9 530-turn session, opening a hit at turn 9 525 fell from 14.5 s to 114 ms, and typing from 110-285 ms a letter to 33-84 ms. The searchable text is folded once at load rather than per keystroke, a search narrows to the previous hits while the query grows, and the cards are memoised.
